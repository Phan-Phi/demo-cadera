...
let servicesDetailSchema = object({
  id: number().required().positive().integer(),
  meta: META_ITEM,
  title: string().required(),
  last_published_at: string().required(),
...
export type ServicesDetailPage = InferType<typeof servicesDetailSchema>;

export type ServiceListingPage = InferType<typeof servicesPageSchema>;
